...
extern void test_fatal_with_message();
extern void test_without_logtag();
extern void test_alogv();

int main(int argc, char **argv) {
    bool do_fatal = false;
    bool do_fatal_with_message = false;

    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "fatal") == 0)
            do_fatal = true;
        if (strcmp(argv[i], "fatal_with_message") == 0)
            do_fatal_with_message = true;
    }
    ...
    if (do_fatal) {
        test_fatal();
    } else if (do_fatal_with_message) {
        test_fatal_with_message();
    } else {
        test_function_cpp();
        test_function_c();
        test_without_logtag();
